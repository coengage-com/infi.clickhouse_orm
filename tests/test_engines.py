...
class EnginesTestCase(_EnginesHelperTestCase):
    ...
    def test_merge(self):
        ...
        self.database.insert(
            [
                TestModel1(
                    date="2017-01-01", event_id=1, event_group=1, event_count=1, event_version=1
                )
            ]
        )
        self.database.insert(
            [
                TestModel2(
                    date="2017-01-02", event_id=2, event_group=2, event_count=2, event_version=2
                )
            ]
        )
...
class DistributedTestCase(_EnginesHelperTestCase):
    ...
    def _test_insert_select(
        ...
        # event_uversion is materialized field. So * won't select it and it will be zero
        res = self.database.select(
            "SELECT *, event_uversion FROM $table ORDER BY event_id", model_class=to_select
        )
