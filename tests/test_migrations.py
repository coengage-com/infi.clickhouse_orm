...
from infi.coengage_clickhouse_orm.database import Database, ServerError
from infi.coengage_clickhouse_orm.engines import *
from infi.coengage_clickhouse_orm.fields import *
from infi.coengage_clickhouse_orm.migrations import MigrationHistory
from infi.coengage_clickhouse_orm.models import BufferModel, Constraint, Index, Model
...
class MigrationsTestCase(unittest.TestCase):
    ...
    def test_migrations(self):
        ...
        self.assertEqual(
            self.get_table_fields(MaterializedModel), [("date_time", "DateTime"), ("date", "Date")]
        )
        self.database.migrate("tests.sample_migrations", 9)
        self.assertTrue(self.table_exists(AliasModel))
        self.assertEqual(
            self.get_table_fields(AliasModel), [("date", "Date"), ("date_alias", "Date")]
        )
        ...
        data = [
            item.f1
            for item in self.database.select(
