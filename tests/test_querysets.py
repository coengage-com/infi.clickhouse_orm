...
from infi.coengage_clickhouse_orm.query import Q
...
class QuerySetTestCase(TestCaseWithData):
    ...
    def test_filter_with_q_objects(self):
        ...
        self._test_qs(
            qs.filter(Q(first_name__in=["Warren", "Whilemina", "Whitney"]) & Q(height__gte=1.7)), 3
        )
        self._test_qs(
            qs.filter(
                (
        ...
        self._test_qs(
            qs.filter(
                ...
                | Q(first_name="Beatrice") & Q(last_name="Gregory")
            ),
            2,
        )
        self._test_qs(
            qs.filter(Q(first_name="Courtney") | Q(first_name="Cassady") & Q(last_name="Knapp")), 3
        )
...
class AggregateTestCase(TestCaseWithData):
    ...
    def test_aggregate_with_implicit_grouping(self):
        qs = Person.objects_in(self.database).aggregate(
            "first_name", average_height="avg(height)", count="count()"
        )
        print(qs.as_sql())
    ...
    def test_aggregate_with_wrong_grouping(self):
        with self.assertRaises(AssertionError):
            Person.objects_in(self.database).aggregate(
                weekday="toDayOfWeek(birthday)", count="count()"
            ).group_by("first_name")
    ...
    def test_limit_by(self):
        ...
        qs = (
            Person.objects_in(self.database)
            .filter(height__gt=1.67)
            .order_by("height", "first_name")
        )
