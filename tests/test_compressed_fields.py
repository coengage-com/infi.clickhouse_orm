...
from infi.coengage_clickhouse_orm.utils import parse_tsv
...
class CompressedFieldsTestCase(unittest.TestCase):
    ...
    def test_to_dict(self):
        instance = CompressedModel(
            date_field="1973-12-06", int64_field="100", float_field="7", array_field="[a,b,c]"
        )
        ...
        self.assertDictEqual(
            instance.to_dict(
                include_readonly=False, field_names=("int64_field", "alias_field", "datetime_field")
            ),
            {
                "int64_field": 100,
                "datetime_field": datetime.datetime(1970, 1, 1, 0, 0, 0, tzinfo=pytz.utc),
