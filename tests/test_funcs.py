...
from infi.coengage_clickhouse_orm.database import ServerError
from infi.coengage_clickhouse_orm.funcs import F
from infi.coengage_clickhouse_orm.utils import NO_VALUE
...
class FuncsTestCase(TestCaseWithData):
    ...
    def test_func_to_sql(self):
        ...
        # String args
        self.assertEqual(
            F("func", "Wendy's", u"Wendy's").to_sql(), "func('Wendy\\'s', 'Wendy\\'s')"
        )
        # Numeric args
        ...
        self.assertEqual(
            F("func", pytz.timezone("Europe/Athens")).to_sql(), "func('Europe/Athens')"
        )
        # Null args
    ...
    def test_date_functions(self):
        ...
        self._test_func(
            F.toISOYear(dt, "Europe/Athens"), 2019
        )  # 2018-12-31 is ISO year 2019, week 1, day 1
        ...
        self._test_func(
            F.toStartOfFiveMinute(dt), datetime(2018, 12, 31, 11, 20, 0, tzinfo=pytz.utc)
        )
        self._test_func(
            F.toStartOfFifteenMinutes(dt), datetime(2018, 12, 31, 11, 15, 0, tzinfo=pytz.utc)
        )
        ...
        self._test_func(
            F.toStartOfTenMinutes(dt), datetime(2018, 12, 31, 11, 20, 0, tzinfo=pytz.utc)
        )
        ...
        self._test_func(
            F.now()
            + F.toIntervalSecond(3)
            + F.toIntervalMinute(3)
            + F.toIntervalHour(3)
            + F.toIntervalDay(3)
        )
        self._test_func(
            ...
            + F.toIntervalWeek(3)
            + F.toIntervalMonth(3)
            + F.toIntervalQuarter(3)
            + F.toIntervalYear(3)
        )
        self._test_func(
            F.now() + F.toIntervalSecond(3000) - F.toIntervalDay(3000)
            == F.now() + timedelta(seconds=3000, days=-3000)
        )
    ...
    def test_date_functions__utc_only(self):
        ...
        self._test_func(
            F.toTime(dt, "Europe/Athens"), athens_tz.localize(datetime(1970, 1, 2, 13, 22, 33))
        )
        self._test_func(
            F.toTime(dt, athens_tz), athens_tz.localize(datetime(1970, 1, 2, 13, 22, 33))
        )
        self._test_func(
    ...
    def test_type_conversion_functions__utc_only(self):
        if self.database.server_timezone != pytz.utc:
            raise unittest.SkipTest("This test must run with UTC as the server timezone")
        self._test_func(
            F.toDateTime("2018-12-31 11:22:33"), datetime(2018, 12, 31, 11, 22, 33, tzinfo=pytz.utc)
        )
        self._test_func(
    ...
    def test_bitmap_functions(self):
        ...
        self._test_func(
            F.bitmapToArray(F.bitmapAnd(F.bitmapBuild([1, 2, 3]), F.bitmapBuild([3, 4, 5]))), [3]
        )
        self._test_func(
        ...
        self._test_func(
            F.bitmapAndCardinality(F.bitmapBuild([1, 2, 3]), F.bitmapBuild([3, 4, 5])), 1
        )
        self._test_func(
            F.bitmapOrCardinality(F.bitmapBuild([1, 2, 3]), F.bitmapBuild([3, 4, 5])), 5
        )
        self._test_func(
            F.bitmapXorCardinality(F.bitmapBuild([1, 2, 3]), F.bitmapBuild([3, 4, 5])), 4
        )
        self._test_func(
            F.bitmapAndnotCardinality(F.bitmapBuild([1, 2, 3]), F.bitmapBuild([3, 4, 5])), 2
        )
    ...
    def test_ip_funcs(self):
        ...
        self._test_func(
            F.IPv6NumToString(F.IPv4ToIPv6(F.IPv4StringToNum("192.168.0.1"))), "::ffff:192.168.0.1"
        )
    ...
    def test_aggregate_funcs(self):
        ...
        self._test_aggr(
            F.round(F.avg(Person.height), 4), sum(p.height for p in self._sample_data()) / 100
        )
        ...
        self._test_aggr(
            F.round(F.sum(Person.height), 4), sum(p.height for p in self._sample_data())
        )
    ...
    def test_aggregate_funcs__or_default_if(self):
        self._test_aggr(
            F.argMinOrDefaultIf(Person.first_name, Person.height, Person.last_name > "Z")
        )
    ...
    def test_quantile_funcs(self):
        ...
        self._test_aggr(
            F.quantilesTimingWeightedOrDefaultIf(0.9, 0.95, 0.99)(Person.height, weight_expr, cond)
        )
    ...
    def test_top_k_funcs(self):
        ...
        self._test_aggr(
            F.topKWeightedOrDefaultIf(3)(Person.height, weight_expr, Person.last_name > "H")
        )
