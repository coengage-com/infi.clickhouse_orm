...
from infi.coengage_clickhouse_orm import Database, F
...
def database_view(db_name):
    ...
    tables = (
        ...
        .aggregate(
            ...
            ratio=F.sum(ColumnsTable.data_uncompressed_bytes)
            / F.sum(ColumnsTable.data_compressed_bytes),
        )
    )
...
def table_view(db_name, tbl_name):
    ...
    return render_template(
        ...
        columns=columns,
        piechart=columns_piechart(
            db, tbl_name, "data_compressed_bytes", value_formatter=bytes_formatter
        ),
    )
